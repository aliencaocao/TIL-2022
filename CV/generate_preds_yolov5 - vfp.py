...
image_info = json.loads(open("data/finals/qualifiers_finals_no_annotations.json").read())["images"]
filename_to_img = {}
...
all_preds = []
for pred_txt_path in Path("../../yolov5/runs/detect/exp/labels").glob("*.txt"):
  with open(str(pred_txt_path)) as pred_txt:
    for pred_line in pred_txt:
      ...
      all_preds.append({
        "image_id": img["id"],
        "bbox": [x, y, w, h],
        #"category_id": class_idx if class_idx == 1 else class_idx + 2,
